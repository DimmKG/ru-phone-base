...
| `npm run diff:data -- --old <dir> --output <dir>`                           | Сравнивает старый снапшот (`<dir>/data/*.json` + `<dir>/reports/*.json`) с текущим `src/data`/`src/reports`, пишет `stats.json` и детальные JSON-файлы диффа, печатает краткую сводку в консоль. Не публикуется в пакете, живёт в `src/bin/diff-dataset.ts` + `src/build/diff/`. |
...
5. **Посчитать дифф/статистику**:
   ...
   ```bash
   npm run diff:data -- --old /tmp/old-snapshot --output /tmp/diff-output
   ```
