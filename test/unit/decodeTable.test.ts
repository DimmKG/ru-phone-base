...
import type { Dataset, OperatorsIndex } from '../../src/types.js';
...
const operators: OperatorsIndex = { '1234567890': 'Operator A' };
...
describe('decodeTable', () => {
  it('decodes a regular block with resolved region and timezone', () => {
    ...
    const [alloc] = decodeTable(table, 'fixed', timezones, operators);
  ...
  it('strips the "all-russia" pseudo-slug from regions but keeps nationwide true', () => {
    ...
    const [alloc] = decodeTable(table, 'fixed', {}, operators);
    expect(alloc.nationwide).toBe(true);
